...
def generate_text(
    ...
    logger.info(f"LLM Completion: {completion.choices[0].message.content}")
...
def generate_text_from_vlm(
    ...
    completion_string = completion.json()["choices"][0]["message"]["content"]
    logger.info(f"VLM Completion: {completion_string}")
...
def main_loop(
    ...
    for _ in tqdm.tqdm(range(args.max_actions)):
        ...
        # try/except workaround to continue even if response errors such as image prompt filtering/censoring
        try:
            match action:
                case "propose_section":
                    ...
                    section_title_response = generate_text(
                        "Executive Summary:\n"
                        + "\n".join(document["executive_summary"])
                        + "\n\nPropose a new section title for the paper with a boring, standard name. If not yet mentioned in the executive summary, propose 'Abstract'. Respond in json format. It should be as short as possible, with a maximum of 5 words. The key 'title' should contain the answer",
                        logger,
                ...
                case "write_section" if document["sections"]:
                    ...
                    if not isinstance(
                        proposal_title_index_in_empty_sections, int
                    ) or proposal_title_index_in_empty_sections >= len(empty_sections):
                ...
                case "edit_section" if document["sections"]:
                    ...
                    if not isinstance(
                        proposal_title_index_in_filled_sections, int
                    ) or proposal_title_index_in_filled_sections >= len(filled_sections):
                        # Skip if invalid title chosen
                        continue
                    ...
                    edited_content = generate_text(
                        "Executive Summary:\n"
                        + "\n".join(document["executive_summary"])
                        + f"\n\nRewrite this section following inspiration on the current state of the overall paper from the executive summary of the whole paper in the current state: {document['sections'][title_index]['content']}.\n\nRespond with only the LaTeX code for the rewritten version of this section.",
                        logger,
                        api_engine=args.engine,
                    )
                ...
                case "generate_figure":
                    ...
                    image_gen_prompt_response = generate_text(
                        ...
                        + " Tell me in json dict format only which prompt should be used to get an image for an interesting new/unseen figure to add to the current paper state, following inspiration from the executive summary of the whole paper in the current state. The key 'image_prompt' should contain the answer.",
                        logger,
                        api_engine=args.engine,
                        json_mode=True,
                    )
...
def parse_args():
    ...
    parser.add_argument(
        "--random-action",
        action="store_true",
        help="If True, select actions randomly",
    )
